...
class route extends \WP_REST_Posts_Controller {
	...
	public function the_search( $request ) {
		...
		foreach ( $query_result as $post ) {
	 	...
	 	/*--- Start Customization --*/
		return $this->create_response( $request, $posts, $search->found_posts );
		/*--- end Customization --*/
	}
	 ...
	 */
	public function create_response( $request, $data, $total_posts ) {
		$response    = rest_ensure_response( $data );
		$response->header( 'X-WP-Total', (int) $total_posts );
		$max_pages = ceil( $total_posts / $request['posts_per_page'] );
